...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZRATRK_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZRATRK_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZRATRK_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZRATRK_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZRATRK_SURROGATE_ID_SEQUENCE FOR SATURN.SZRATRK_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZRATRK_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZRATRK
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZRATRK_SURROGATE_ID IS NULL THEN
            :NEW.SZRATRK_SURROGATE_ID := SZRATRK_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZRATRK_VERSION :=0;
        ELSE
        IF :NEW.SZRATRK_VERSION = :OLD.SZRATRK_VERSION THEN
            :NEW.SZRATRK_VERSION := :OLD.SZRATRK_VERSION + 1;
        END IF;
        END IF;
    END;
/
