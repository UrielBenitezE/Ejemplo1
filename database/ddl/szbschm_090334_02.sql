...
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *  1. Add comments to table and columns.                                       MKU 26-OCT-2015 *
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *                                                                                              *
...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZBSCHM_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZBSCHM_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZBSCHM_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZBSCHM_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZBSCHM_SURROGATE_ID_SEQUENCE FOR SATURN.SZBSCHM_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZBSCHM_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZBSCHM
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZBSCHM_SURROGATE_ID IS NULL THEN
            :NEW.SZBSCHM_SURROGATE_ID := SZBSCHM_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZBSCHM_VERSION :=0;
        ELSE
        IF :NEW.SZBSCHM_VERSION = :OLD.SZBSCHM_VERSION THEN
            :NEW.SZBSCHM_VERSION := :OLD.SZBSCHM_VERSION + 1;
        END IF;
        END IF;
    END;
/
