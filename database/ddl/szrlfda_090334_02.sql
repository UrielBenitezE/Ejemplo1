...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZRLFDA_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZRLFDA_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZRLFDA_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZRLFDA_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZRLFDA_SURROGATE_ID_SEQUENCE FOR SATURN.SZRLFDA_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZRLFDA_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZRLFDA
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZRLFDA_SURROGATE_ID IS NULL THEN
            :NEW.SZRLFDA_SURROGATE_ID := SZRLFDA_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZRLFDA_VERSION :=0;
        ELSE
        IF :NEW.SZRLFDA_VERSION = :OLD.SZRLFDA_VERSION THEN
            :NEW.SZRLFDA_VERSION := :OLD.SZRLFDA_VERSION + 1;
        END IF;
        END IF;
    END;
/
