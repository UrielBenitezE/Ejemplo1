...
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *  1. Add comments to table and columns.                                       MHI  15/OCT/2024 *
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *  AUDIT TRAIL: END                                                                            *
REM *                                                                                              *
REM ************************************************************************************************
...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZVABJR_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZVABJR_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZVABJR_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZVABJR_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZVABJR_SURROGATE_ID_SEQUENCE FOR SATURN.SZVABJR_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZVABJR_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZVABJR
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZVABJR_SURROGATE_ID IS NULL THEN
            :NEW.SZVABJR_SURROGATE_ID := SZVABJR_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZVABJR_VERSION :=0;
        ELSE
        IF :NEW.SZVABJR_VERSION = :OLD.SZVABJR_VERSION THEN
            :NEW.SZVABJR_VERSION := :OLD.SZVABJR_VERSION + 1;
        END IF;
        END IF;
    END;
/
