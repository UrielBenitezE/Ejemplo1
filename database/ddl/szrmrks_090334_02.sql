...
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *  1. Add comments to table and columns.                                       MKU 20-NOV-2015 *
REM *  --------------------------------------------------------------------------- --- ----------- *
REM *                                                                                              *
...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZRMRKS_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZRMRKS_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZRMRKS_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZRMRKS_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZRMRKS_SURROGATE_ID_SEQUENCE FOR SATURN.SZRMRKS_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZRMRKS_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZRMRKS
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZRMRKS_SURROGATE_ID IS NULL THEN
            :NEW.SZRMRKS_SURROGATE_ID := SZRMRKS_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZRMRKS_VERSION :=0;
        ELSE
        IF :NEW.SZRMRKS_VERSION = :OLD.SZRMRKS_VERSION THEN
            :NEW.SZRMRKS_VERSION := :OLD.SZRMRKS_VERSION + 1;
        END IF;
        END IF;
    END;
/
