...
REM ****************************************************************************
WHENEVER SQLERROR CONTINUE;
    DROP TRIGGER SATURN.SZ_SZRSCHM_SURROGATE_ID;
    DROP SEQUENCE SATURN.SZRSCHM_SURROGATE_ID_SEQUENCE;
    DROP PUBLIC SYNONYM SZRSCHM_SURROGATE_ID_SEQUENCE;

    CREATE SEQUENCE SATURN.SZRSCHM_SURROGATE_ID_SEQUENCE START WITH 1;
    CREATE PUBLIC SYNONYM SZRSCHM_SURROGATE_ID_SEQUENCE FOR SATURN.SZRSCHM_SURROGATE_ID_SEQUENCE;


    CREATE OR REPLACE TRIGGER SATURN.SZ_SZRSCHM_SURROGATE_ID 
    BEFORE INSERT OR UPDATE ON SATURN.SZRSCHM
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
        IF :NEW.SZRSCHM_SURROGATE_ID IS NULL THEN
            :NEW.SZRSCHM_SURROGATE_ID := SZRSCHM_SURROGATE_ID_SEQUENCE.NEXTVAL;
        END IF;
        :NEW.SZRSCHM_VERSION :=0;
        ELSE
        IF :NEW.SZRSCHM_VERSION = :OLD.SZRSCHM_VERSION THEN
            :NEW.SZRSCHM_VERSION := :OLD.SZRSCHM_VERSION + 1;
        END IF;
        END IF;
    END;
/
